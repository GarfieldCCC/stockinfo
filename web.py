...
def func(interval):
    ...
    while current_time_between("09:30", "11:31") or current_time_between("13:00", "15:01"):
        ...
        if time.strftime("%H:%M") == "11:30":
            plotly.offline.plot(fig, filename="Excel/" + time.strftime("%Y-%m-%d") + "/Morning.html", auto_open=False)
        if time.strftime("%H:%M") == "15:00":
            plotly.offline.plot(fig, filename="Excel/" + time.strftime("%Y-%m-%d") + "/Afternoon.html", auto_open=False)
        return fig
    func(interval)
